...
$sqll = "SELECT acesso_descricao FROM acesso";
$result2 = $conn->query($sqll);
if ($result2->num_rows > 0) {
		// output data of each row
		$array2 = array();
		while($row1 = $result2->fetch_assoc()) {
		$array2[] = $row1["acesso_descricao"];
		}
}
        $descri1=$array2[0];
		$descri2=$array2[1];
...
?>
<form action="acessorel.php" method="POST">
...
<?php
	if(empty($_POST['id']) && empty($_POST['dataI']) && empty($_POST['dataF'])){
		$sql = "SELECT acesso_id, acesso_data, acesso_nome FROM acesso_rel ORDER BY acesso_sec DESC limit 10";
		echo "Ultimas 10 Ações";
	}elseif(isset($_POST['id']) && empty($_POST['dataI']) && empty($_POST['dataF'])){
		$sql = "SELECT acesso_id, acesso_data, acesso_nome FROM acesso_rel WHERE acesso_id = '".$_POST['id']."' ORDER BY acesso_sec DESC limit 10";
		echo "Ultimas 10 Ações do acesso "; if($_POST['id']==1){echo $descri1;}else{echo $descri2;}
	}elseif(empty($_POST['id']) && isset($_POST['dataI']) && isset($_POST['dataF'])){
		$sql = "SELECT acesso_id, acesso_sec, acesso_nome, acesso_data FROM acesso_rel WHERE acesso_data >= '".$_POST['dataI']."' AND acesso_data <= '".$_POST['dataF']."' ORDER BY  acesso_sec";
		...
		$sql = "SELECT acesso_id, acesso_sec, acesso_nome, acesso_data FROM acesso_rel WHERE acesso_data >= '".$_POST['dataI']."' AND acesso_data <= '".$_POST['dataF']."' AND acesso_id = '".$_POST['id']."' ORDER BY  acesso_sec";
		$dt = Date('d/m/Y H:i:s',  strtotime($_POST['dataI']));
		$dt2 = Date('d/m/Y H:i:s',  strtotime($_POST['dataF']));
		echo "Ações do periodo ".$dt." até " .$dt2." do acesso "; if($_POST['id']==1){echo $descri1;}else{echo $descri2;}
	}else{
		$sql = "SELECT acesso_id, acesso_data, acesso_nome FROM acesso_rel ORDER BY acesso_sec DESC limit 10";
		echo "Ultimas 10 Ações";
	}
...
if ($result->num_rows > 0) {
	...
	<tr><td><h4>DESCRIÇÃO</h4></td><td al><h4>DATA</h4></td><td><h4>USUARIO</h4></td></tr>
	<?php
	while($row = $result->fetch_assoc()) {
        
	$data = Date('d/m/Y H:i:s',  strtotime($row["acesso_data"]));
	$descr = $descri2;
	if(	$row["acesso_id"]==1){
	$descr = $descri1;	
	}
    ...
    echo "<tr><td>".$descr. "</td><td>" .$data. "</td><td>" .$row["acesso_nome"]. "</td></tr>";
    ...
    echo "Não foi encontrado nenhum resultado nesta busca!!";
	 ...
	 ?>
	 <tr>
		<td colspan="3"><a class="botaocadper" href="acesso.php">Voltar</a></td>
	 </tr>
	 </table>
